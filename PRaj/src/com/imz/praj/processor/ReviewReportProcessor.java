...
public class ReviewReportProcessor extends BaseInitializer implements RequestProcessorInterface {
    ...
    public ReviewReportProcessor(ResponseMessage responseMessage) {
        this.responseMessage = responseMessage;
    }

    public ReviewReportProcessor() {}
        ...
        try {
            ...
            if (deviceDataByDist != null) {
                ...
                for (Map.Entry<Long, Map<String, String>> entry : deviceDataByDist.entrySet()) {
                    ...
                    int funcIot = reportMap.size();

                    RedisToMongoDaoImpl redisToMongoDao = new RedisToMongoDaoImpl();
                    Calendar cal = Calendar.getInstance();
                    cal.add(Calendar.DATE,-1);
                    String date = getDbPattern().format(cal.getTime());
                    nonFuncSch = redisToMongoDao.nonFuncCount(accountList, reportMap,"NFS",date,date);

//                    nonFuncIOT = redisToMongoDao.nonFuncIotCount(deviceMap,"NFI",date,date);
//                    nonFuncSch = deviceMap.size()-nonFuncIOT-funcIot;
                    nonFuncIOT = deviceMap.size() -nonFuncSch -funcIot ;


                    distObj.put("total", target);
                    distObj.put(Constants.NODEQUANTITY, deviceMap.size());
                    distObj.put("funcsch", funcIot);
