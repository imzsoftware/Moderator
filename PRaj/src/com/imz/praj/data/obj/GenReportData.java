...
public class GenReportData extends BaseInitializer{
    private String date;
    ...
    public static List<GenReportData> createList(FindIterable<Document> resultList) {
        List<GenReportData> alertLogList = new ArrayList<GenReportData>();
        try {
            for (MongoCursor<Document> mongoCursor = resultList.iterator(); mongoCursor.hasNext(); ) {
                Document doc = mongoCursor.next();
                GenReportData data = new GenReportData();
    ...
    public static List<GenReportData> createGenReport(FindIterable<Document> resultList) {
        List<GenReportData> alertLogList = new ArrayList<GenReportData>();
        try {
    ...
    public static Map<Long, GenReportData> wardGenReport(FindIterable<Document> resultList) {
        Map<Long, GenReportData> reportDataMap =  new HashMap<>();
        ...
        try {
            ...
            while (mongoCursor.hasNext()) {
                ...
                if(reportDataMap.containsKey(doc.getLong(Constants.NODEDEVICEID))){
                    data = reportDataMap.get(doc.getLong(Constants.NODEDEVICEID));
                    total = data.getTotalQty() + doc.getLong(Constants.NODETOTAL);
                ...
                reportDataMap.put(doc.getLong(Constants.NODEDEVICEID),data);
            }
