...
public class InitBean extends BaseInitializer {
    ...
    private Boolean initBean() {
        ...
        try {
            ...
            BeanFactory beanFactory = BeanFactoryManager.manager("producer.xml");
            ...
            String tpClients = getAppConfig().getTpClients();

            Map<String, Integer> map = new HashMap<String, Integer>();
            if(tpClients != null) {
                StringTokenizer stn = new StringTokenizer(tpClients,":;");
                while (stn.hasMoreTokens()) {
                    map.put(stn.nextToken(), Integer.parseInt(stn.nextToken()));
                }
            }
            getAppConfig().setClientMap(map);
/*            if(coreConfig != null){
...
/*        } catch (SQLException e) {
    ...
    public void redisPoolInitialized(){
        try {
            /*RedisURI redisUri = RedisURI.Builder.redis(getAppConfig().getRedisHost())
                    .withPassword(getAppConfig().getRedisPass()).build();
            client = RedisClusterClient.create(RedisURI.create(redisUri));*/
    ...
    public void mongoDbConnection(){
        ...
        String [] hostArr = getAppConfig().getMongoHost().split(",");
